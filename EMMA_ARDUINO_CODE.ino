...
void loop() {
  ...
  serialData.Get(valsRec); //Angles recieved from Arduino
  LServo.write(valsRec[0]); //Set the servo the received position
  RServo.write(valsRec[1]); //Set the servo the received position
  HServo.write(valsRec[2]); //Set the servo the received position
}
